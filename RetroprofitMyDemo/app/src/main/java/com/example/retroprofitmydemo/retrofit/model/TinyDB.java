package com.example.retroprofitmydemo.retrofit.model;
...
//    }
    ...
    public List<OdrderDetail> getOrderList(String key) {

        Gson gson = new Gson();

        ArrayList<String> objStrings = getListString(key);
        ArrayList<OdrderDetail> objects = new ArrayList<>();
        for (String jObjString : objStrings) {
            Log.e("TAG", "getPaymentListResponseModel: jObjString--&gt;"+jObjString );
            OdrderDetail value = gson.fromJson(jObjString,OdrderDetail.class);
            objects.add(value);
        }
        Log.e("TinyDB","fav list"+objects);
        return objects;
    }


    public void putorderList(String key, List<OdrderDetail> objArray) {

        checkForNullKey(key);
        Gson gson = new Gson();
        ArrayList<String> objStrings = new ArrayList<String>();
        for (OdrderDetail obj : objArray) {
